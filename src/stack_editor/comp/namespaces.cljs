...
(defn on-edit-ns [namespace']
  (fn [e dispatch!] (dispatch! :collection/edit [:namespaces namespace']) (focus!)))
  ...
  (fn [state mutate!]
    (div
     ...
     (div
      ...
      (comp-space "8px" nil)
      (div
       {:style widget/button, :event {:click (on-add-namespace mutate! state)}}
       (comp-text "add" nil)))
     (comp-space nil "16px")
     (interpose-spaces
      (div
            ...
            (map
             (fn [entry]
               (let [ns-name (first entry)]
                 [ns-name
                  (div
                   {:style widget/entry-line, :event {:click (on-edit-ns ns-name)}}
                   (comp-text ns-name nil))])))))
      {:height "8px"}))))

(def comp-namespaces (create-comp :namespaces init-state update-state render))
